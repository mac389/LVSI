...
for (pathologist_one, pathologist_two) in itertools.combinations(pathologists,2):
	...
	df_one = pd.read_excel('stains.xls',pathologist_one,parse_cols=cols_with_grades, convert_float=False)
	df_two = pd.read_excel('stains.xls',pathologist_two,parse_cols=cols_with_grades, convert_float=False)
...
json.dump(lvsi,open('lvsi-stains-grades.json','wb'))
