...
for j,pathologist in enumerate(pathologists):
	...
	histology_grade = pd.read_excel('../data/no-stain.xls',pathologist,parse_cols=cols_with_grades,convert_float=False)
	lvsi = pd.read_excel('../data/no-stain.xls',pathologist,parse_cols=cols_with_lvsi,convert_float=False)
	...
	for histolog_rating,lvsi_rating in zip(histology_ratings,lvsi_ratings):
   	...
   	kappas[pathologist] = cohens_kappa(contingency_table[j,:,:].squeeze()).kappa
